...
- **Free Postgres expired.** Render's free databases expire 30 days after
  ...
  period, keeping your data), or delete it, create a new free database, and
  point the web service's `DATABASE_URL` env var at the new database's
  connection string (a blueprint re-sync also works). A fresh database starts
  empty — `migrate()` re-applies the schema on boot — but free instances have
  no backups, so user data is gone.
